  ...
  return (
    <footer className="relative py-8 md:py-12 px-6 bg-gradient-to-r from-[#0a0e27] to-[#1a1f3a] border-t border-cyan-500/10">
      <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col gap-2">
              {numerosWhatsApp.map((item, index) => (
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 text-cyan-400 hover:text-cyan-300 font-semibold transition-colors"
                >
                  ...
                  <span>{item.nome}: ({item.numero.slice(2, 4)}) {item.numero.slice(4, 9)}-{item.numero.slice(9)}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        >
          <p>&copy; {currentYear} Alpha Recargas. Todos os direitos reservados.</p>
        </motion.div>
      </div>
    </footer>
  )
