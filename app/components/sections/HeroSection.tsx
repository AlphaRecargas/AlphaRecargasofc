...
export function HeroSection() {
  ...
  return (
    <section className="relative bg-gradient-to-br from-[#0a0e27] via-[#0f1535] to-[#1a1f3a] overflow-hidden pt-20 pb-4">
                ...
                <div className="space-y-3">
                  {contatos.map((contato, index) => (
                    <motion.button
                      key={contato.numero}
                      type="button"
                      onClick={() => handleWhatsAppClick(contato.numero)}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: index * 0.1 }}
                      className="w-full flex items-center justify-between p-4 rounded-xl bg-white/5 border border-cyan-500/20 hover:border-cyan-400/50 hover:bg-cyan-500/10 transition-all duration-300 group cursor-pointer"
                    >
