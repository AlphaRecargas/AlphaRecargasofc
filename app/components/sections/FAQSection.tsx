    ...
    <section className="relative py-16 md:py-24 px-6 bg-gradient-to-br from-[#0f1535] via-[#1a1f3a] to-[#0a0e27] overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        >
          ...
          {/* Popup com os números - CORRIGIDO */}
          <AnimatePresence>
            {mostrarNumeros && (
              <>
                {/* Overlay DENTRO do AnimatePresence e ANTES do popup */}
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                ...
                {/* Popup com z-50 fica POR CIMA do overlay z-40 */}
                <motion.div
                  initial={{ opacity: 0, y: 10, scale: 0.95 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: 10, scale: 0.95 }}
                  transition={{ duration: 0.2 }}
                  className="absolute left-1/2 -translate-x-1/2 bottom-full mb-4 w-72 bg-[#1a1f3a] border border-cyan-500/30 rounded-xl shadow-2xl overflow-hidden z-50"
                >
                  ...
                  {numerosWhatsApp.map((item, index) => (
                    <a
                      key={index}
                      href={`https://wa.me/${item.numero}?text=${encodeURIComponent(item.mensagem)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full px-4 py-3 flex items-center gap-3 hover:bg-cyan-500/10 transition-colors text-left cursor-pointer"
                    >
